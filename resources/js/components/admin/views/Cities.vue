              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="myModalLabel"> {{trans('lang.states')}} </h5>
        ...
        <div class="card">
            <div class="card-body">
                <div class="row g-2">
                    ...
                    <div class="col-sm-3">
                        <div class="search-box">
                            <v-select required v-model="form_filter.state_id" :options="states" :reduce="state => state.id" label="lang_name"></v-select>
                        </div>
                    </div>
                    <!--end col-->
    ...
    <table class="table table-nowrap mb-0">
        <thead class="table-light">
            <tr>
                ...
                <th scope="col">{{trans('lang.city')}}</th>
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get('/admin/admin_api/cities')
                 .then((resp)=>{
        ...
        filter(){
            this.$root.loading=true
            this.form_filter.post('/admin/admin_api/filter_cities')
                 .then((resp)=>{
        ...
        create(){
            this.form.post('/admin/admin_api/cities')
                .then((resp)=>{
                    Fire.$emit('Load')
                    //alert
        ...
        update(){
            this.form.put('/admin/admin_api/cities/'+this.form.id)
                .then(()=>{
                    Fire.$emit('Load')
                    //alert
        ...
        del(){
          this.form.delete('/admin/admin_api/cities/'+this.item.id)
                        .then(()=>{ 
                         Fire.$emit('Load');
         ...
         getResults(page = 1) {
            ...
            axios.get('/admin/admin_api/cities?page=' + page)
                .then(({data}) => (
                    this.tableData = data
                ))
        },
        searching(){
               let query=this.$root.search;
                this.$root.loading=true
              if(query.length != 0){                
              axios.get('/admin/admin_api/cities/search/'+query)
                        .then((response)=>{  
