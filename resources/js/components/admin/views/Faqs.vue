              ...
              <div class="modal-dialog modal-lg modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="myModalLabel"> {{trans('lang.faqs')}} </h5>
                      ...
                      <div class="modal-body">
                          <form @submit.prevent="editMode?update():create()">
                              <div class="row">
                                  ...
                                  <div class="col-lg-4" v-for="i,index in this.question_count" :key="index">
                                      <div class="mb-3">
                                          <label for="teammembersName" class="form-label">{{trans('lang.questtion_'+question_count[index])}}</label>
                                          <input  :class="{ 'is-invalid': form.errors.has('question') } " v-model="form.question[question_count[index]]"  class="form-control" >
                                           <div class="text-danger" v-if="form.errors.has('question')" v-html="form.errors.get('question')" />
                                      </div>
                                  </div>
                             
                                  
                                  <div class="col-lg-12" v-for="ii,index2 in this.answer_count" :key="index2">
                                      <div class="mb-3">
                                          <label for="teammembersName" class="form-label">{{trans('lang.answer_'+answer_count[index2])}}</label>
                                          <vue-editor v-model="form.answer[answer_count[index2]]"></vue-editor> 
                                           <div class="text-danger" v-if="form.errors.has('answer')" v-html="form.errors.get('answer')" />
                                      </div>
                                  </div>
    ...
    <table class="table  mb-0">
        <thead class="table-light">
            <tr>
                ...
                <th scope="col">{{trans('lang.question')}}</th>
                <th scope="col">{{trans('lang.answer')}}</th>
                <th width="200" scope="col">Options</th>
            </tr>
        </thead>
        <tbody>
            <tr v-for="item,index in tableData.data" :key="index">
                ...
                <td>{{item.lang_question}}</td>
                <td>  <div class="answer-div" v-html="item.lang_answer">  </div></td>
                <td>
...
export default {
    mounted(){
        ...
        Fire.$on('Load',()=>{
         ...
         this.getData()
         this.question_count=Object.keys(this.form.question);
         this.answer_count=Object.keys(this.form.answer);
    },
    components: {
        VueEditor
    },
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get('/admin/admin_api/faqs')
                 .then((resp)=>{
        ...
        create(){
            this.form.post('/admin/admin_api/faqs')
                .then((resp)=>{
                    Fire.$emit('Load')
                    //alert
        ...
        update(){
            this.form.put('/admin/admin_api/faqs/'+this.form.id)
                .then(()=>{
                    Fire.$emit('Load')
                    //alert
        ...
        del(){
          this.form.delete('/admin/admin_api/faqs/'+this.item.id)
                        .then(()=>{ 
                         Fire.$emit('Load');
         ...
         getResults(page = 1) {
            ...
            axios.get('/admin/admin_api/faqs?page=' + page)
                .then(({data}) => (
                    this.tableData = data
                ))
        },
               ...
               let query=this.$root.search;
                ...
                axios.get('/admin/admin_api/faqs')
                 .then((resp)=>{
                    ...
                    result.lang_question.toLowerCase().includes(query.toLowerCase())
                    );
                    this.$root.loading=false
                    }else{
                        this.getData();
