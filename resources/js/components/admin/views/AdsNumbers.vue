              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="myModalLabel"> {{trans('lang.ads_numbers')}} </h5>
                      ...
                      <div class="modal-body">
                          <form @submit.prevent="editMode?update():create()">
                              <div class="row">
                                  <div class="col-lg-12">
                                      <div class="mb-3">
                                          <label  class="form-label">{{trans('lang.ads_numbers')}}</label>
                                          <input type="number" required :class="{ 'is-invalid': form.errors.has('ads_number') } " v-model="form.ads_number"  class="form-control" >
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get('/admin/admin_api/ads_numbers')
                 .then((resp)=>{
        ...
        create(){
            this.form.post('/admin/admin_api/ads_numbers')
                .then((resp)=>{
                    Fire.$emit('Load')
                    //alert
        ...
        update(){
            this.form.put('/admin/admin_api/ads_numbers/'+this.form.id)
                .then(()=>{
                    Fire.$emit('Load')
                    //alert
        ...
        del(){
          this.form.delete('/admin/admin_api/ads_numbers/'+this.item.id)
                        .then(()=>{ 
                         Fire.$emit('Load');
         ...
         getResults(page = 1) {
            ...
            axios.get('/admin/admin_api/ads_numbers?page=' + page)
                .then(({data}) => (
                    this.tableData = data
                ))
        },
        searching(){
               let query=this.$root.search;
                this.$root.loading=true
              if(query.length != 0){                
              axios.get('/admin/admin_api/ads_numbers/search/'+query)
                        .then((response)=>{  
