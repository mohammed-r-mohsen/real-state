              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="myModalLabel"> {{trans('lang.building_ages')}} </h5>
                              ...
                              <div class="row">
                                  <div class="col-lg-12">
                                      <div class="mb-3">
                                          ...
                                          <input type="text" required :class="{ 'is-invalid': form.errors.has('name') } " v-model="form.name"  class="form-control" >
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get('/admin/admin_api/building_ages')
                 .then((resp)=>{
        ...
        create(){
            this.form.post('/admin/admin_api/building_ages')
                .then((resp)=>{
                    Fire.$emit('Load')
                    //alert
        ...
        update(){
            this.form.put('/admin/admin_api/building_ages/'+this.form.id)
                .then(()=>{
                    Fire.$emit('Load')
                    //alert
        ...
        del(){
          this.form.delete('/admin/admin_api/building_ages/'+this.item.id)
                        .then(()=>{ 
                         Fire.$emit('Load');
         ...
         getResults(page = 1) {
            ...
            axios.get('/admin/admin_api/building_ages?page=' + page)
                .then(({data}) => (
                    this.tableData = data
                ))
        },
        searching(){
               let query=this.$root.search;
                this.$root.loading=true
              if(query.length != 0){                
              axios.get('/admin/admin_api/building_ages/search/'+query)
                        .then((response)=>{  
