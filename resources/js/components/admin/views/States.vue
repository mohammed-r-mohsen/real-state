              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="myModalLabel"> {{trans('lang.cities')}} </h5>
                      ...
                      <div class="modal-body">
                          <form @submit.prevent="editMode?update():create()">
                              <div class="row">
                                  ...
                                  <div class="col-lg-12">
                                      <div class="mb-3">
                                          <label for="teammembersName" class="form-label">{{trans('lang.country')}}</label>
                                          <v-select placeholder="select" required v-model="form.country_id" :options="countries" :reduce="country => country.id" label="lang_name"></v-select>
                                           <div class="text-danger" v-if="form.errors.has('country_id')" v-html="form.errors.get('country_id')" />
                                      </div>
                                  </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="row g-2">
                    ...
                    <div class="col-sm-3">
                        <div class="search-box">
                            <v-select required v-model="form_filter.country_id" :options="countries" :reduce="country => country.id" label="lang_name"></v-select>
                        </div>
                    </div>
                    <!--end col-->
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get('/admin/admin_api/states')
                 .then((resp)=>{
        ...
        filter(){
            this.$root.loading=true
            this.form_filter.post('/admin/admin_api/filter_states')
                 .then((resp)=>{
        ...
        create(){
            this.form.post('/admin/admin_api/states')
                .then((resp)=>{
                    Fire.$emit('Load')
                    //alert
        ...
        update(){
            this.form.put('/admin/admin_api/states/'+this.form.id)
                .then(()=>{
                    Fire.$emit('Load')
                    //alert
        ...
        del(){
          this.form.delete('/admin/admin_api/states/'+this.item.id)
                        .then(()=>{ 
                         Fire.$emit('Load');
         ...
         getResults(page = 1) {
            ...
            axios.get('/admin/admin_api/states?page=' + page)
                .then(({data}) => (
                    this.tableData = data
                ))
        },
