              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="myModalLabel"> {{trans('lang.regions')}} </h5>
                      ...
                      <div class="modal-body">
                          <form @submit.prevent="editMode?update():create()">
                              <div class="row">
                                  ...
                                  <div class="col-lg-12">
                                      <div class="mb-3">
                                          <label for="teammembersName" class="form-label">{{trans('lang.state')}}</label>
                                           <v-select required v-model="form.city_id" :options="cities" :reduce="city => city.id" label="custom_name"></v-select>
        ...
        <div class="card">
            <div class="card-body">
                <div class="row g-2">
                    ...
                    <div class="col-sm-3">
                        <div class="search-box">
                            <v-select required v-model="form_filter.city_id" :options="cities" :reduce="city => city.id" label="lang_name"></v-select>
                        </div>
                    </div>
                    <!--end col-->
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get('/admin/admin_api/regions')
                 .then((resp)=>{
        ...
        get_cities(){
            axios.get('/admin/admin_api/get_cities')
                 .then((resp)=>{
        ...
        filter(){
            this.$root.loading=true
            this.form_filter.post('/admin/admin_api/filter_regions')
                 .then((resp)=>{
        ...
        create(){
            this.form.post('/admin/admin_api/regions')
                .then((resp)=>{
                    Fire.$emit('Load')
                    //alert
        ...
        update(){
            this.form.put('/admin/admin_api/regions/'+this.form.id)
                .then(()=>{
                    Fire.$emit('Load')
                    //alert
        ...
        del(){
          this.form.delete('/admin/admin_api/regions/'+this.item.id)
                        .then(()=>{ 
                         Fire.$emit('Load');
         ...
         getResults(page = 1) {
            ...
            axios.get('/admin/admin_api/regions?page=' + page)
                .then(({data}) => (
                    this.tableData = data
                ))
        },
        searching(){
               let query=this.$root.search;
                this.$root.loading=true
              if(query.length != 0){                
              axios.get('/admin/admin_api/regions/search/'+query)
                        .then((response)=>{  
