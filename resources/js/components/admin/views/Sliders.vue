              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="myModalLabel"> {{trans('lang.sliders')}} </h5>
                      ...
                      <div class="modal-body">
                          <form @submit.prevent="editMode?update():create()">
                              <div class="row">
                                  ...
                                  <div class="col-lg-12">
                                      <div class="mb-3">
                                          <label for="userslider" class="form-label">{{trans('lang.slider')}}</label>
                                          <input :class="{ 'is-invalid': form.errors.has('slider') }" @change="upload" type="file" class="form-control" id="userslider">
                                           <div class="text-danger" v-if="form.errors.has('slider')" v-html="form.errors.get('slider')" />
                                      </div>
                                  </div>
    ...
    <table class="table table-nowrap mb-0">
        <thead class="table-light">
            <tr>
                ...
                <th scope="col">{{trans('lang.slider')}}</th>
        ...
        <tbody>
            <tr v-for="item,index in tableData.data" :key="index">
                ...
                <td><img class="slider-img" v-if="item.slider" :src="item.slider"></td>
                <td>
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get('/admin/admin_api/sliders')
                 .then((resp)=>{
        ...
        upload(e){
          ...
          reader.onload= ()=>{
            this.form.slider=reader.result
          }
          reader.readAsDataURL(file)
        },
        create(){
            this.form.post('/admin/admin_api/sliders')
                .then((resp)=>{
                    Fire.$emit('Load')
                    //alert
        ...
        update(){
            this.form.put('/admin/admin_api/sliders/'+this.form.id)
                .then(()=>{
                    Fire.$emit('Load')
                    //alert
        ...
        del(){
          this.form.delete('/admin/admin_api/sliders/'+this.item.id)
                        .then(()=>{ 
                         Fire.$emit('Load');
         ...
         getResults(page = 1) {
            ...
            axios.get('/admin/admin_api/sliders?page=' + page)
                .then(({data}) => (
                    this.tableData = data
                ))
        },
        searching(){
               let query=this.$root.search;
                this.$root.loading=true
              if(query.length != 0){                
              axios.get('/admin/admin_api/sliders/search/'+query)
                        .then((response)=>{  
