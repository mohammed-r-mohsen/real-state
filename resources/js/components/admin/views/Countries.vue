              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="myModalLabel"> {{trans('lang.countrties')}} </h5>
                      ...
                      <div class="modal-body">
                          <form @submit.prevent="editMode?update():create()">
                              <div class="row">
                                  ...
                                  <div class="col-lg-12">
                                      <div class="mb-3">
                                          <label class="form-label">{{trans('lang.country_code')}}</label>
                                          <input required :class="{ 'is-invalid': form.errors.has('country_code') } " v-model="form.country_code"  class="form-control" id="teammemberscountry_code" >
                                           <div class="text-danger" v-if="form.errors.has('country_code')" v-html="form.errors.get('country_code')" />
                                      </div>
                                  </div>
                                  <div class="col-lg-12">
                                      <div class="mb-3">
                                          <label class="form-label">{{trans('lang.phone_code')}}</label>
                                          <input required :class="{ 'is-invalid': form.errors.has('phone_code') } " v-model="form.phone_code"  class="form-control" id="teammembersphone_code" >
                                           <div class="text-danger" v-if="form.errors.has('phone_code')" v-html="form.errors.get('phone_code')" />
                                      </div>
                                  </div>
    ...
    <table class="table table-nowrap mb-0">
        <thead class="table-light">
            <tr>
                ...
                <th scope="col">{{trans('lang.country_code')}}</th>
                <th scope="col">{{trans('lang.phone_code')}}</th>
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get('/admin/admin_api/countries')
                 .then((resp)=>{
        ...
        create(){
            this.form.post('/admin/admin_api/countries')
                .then((resp)=>{
                    Fire.$emit('Load')
                    //alert
        ...
        update(){
            this.form.put('/admin/admin_api/countries/'+this.form.id)
                .then(()=>{
                    Fire.$emit('Load')
                    //alert
        ...
        del(){
          this.form.delete('/admin/admin_api/countries/'+this.item.id)
                        .then(()=>{ 
                         Fire.$emit('Load');
         ...
         getResults(page = 1) {
            ...
            axios.get('/admin/admin_api/countries?page=' + page)
                .then(({data}) => (
                    this.tableData = data
                ))
        },
